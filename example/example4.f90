program main
    ...
    type(vect_t)                  :: u_span(num_time_pts)
    type(vect_t)                  :: u_chord(num_time_pts)
    type(vect_t)                  :: u_norm(num_time_pts)
    ...
    u_chord = rotate(u_chord, euler)
    u_norm  = rotate(u_norm,  euler)

    do i=1,num_elem
        ax_vect(:,i) = u_span*elem_pos(i)
        le_vect(:,i) = u_span*elem_pos(i) + u_chord*le_pos(i)
        te_vect(:,i) = u_span*elem_pos(i) + u_chord*te_pos(i)
    end do
